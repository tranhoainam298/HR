...
namespace HRWebApp.Controllers
{
    public class PersonalsController : Controller
    {
        ...
        {
            if (!DBWatchdog.DatabaseAlive)
            {
                return Content("❌ Database đang offline");
            }

            int pageSize = 100;

            int totalRows = db.Personals.Count();
            int totalPages = (int)Math.Ceiling((double)totalRows / pageSize);

            var data = db.Personals
                         .AsNoTracking()
            ...
            var model = new PagedResult<Personal>
            {
                Items = data,
                CurrentPage = page,
                TotalPages = totalPages,
                TotalRecords = totalRows // thêm dòng này
            };
